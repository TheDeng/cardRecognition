...
def score_map_loss(y_true_cls, y_pred_cls):
    # beta = 1 - torch.sum(torch.sum(y_true_cls,-1),-1) / y_true_cls.shape[1] / y_true_cls.shape[2] 
    # loss = -beta *  y_true_cls * torch.log(y_pred_cls)
...
def generate_rbox(im_size, polys):
    ...
    return score_map.astype(np.float32), geo_map.astype(np.float32)
...
def to_cuda(varibles):
    return varibles.cuda()

def loss_function(y_true_cls, y_pred_cls, y_true_geo, y_pred_geo):
    # print(y_true_cls)
    # print(y_pred_cls)
    # print(y_true_geo)
    # print(y_pred_geo)
    cls_loss = score_map_loss(y_true_cls, y_pred_cls)
    cls_loss *= 0.01
    ...
    # in_h = torch.min(d3_gt, d3_pred) - torch.max(d1_gt, d1_pred)
    # in_w = torch.min(d2_gt, d2_pred) - torch.max(d4_gt, d4_pred)
    # area_intersect = in_h*in_w
    ...
    # area_union = (d3_gt - d1_gt) * (d2_gt - d4_gt) + \
    #                 (d3_pred - d1_pred) * (d2_pred - d4_pred) - area_intersect
    L_AABB = -torch.log((area_intersect + 1.0)/(area_union + 1.0))
    L_theta = 1 - torch.cos(theta_pred - theta_gt)
    L_g = L_AABB + 20 * L_theta

    return torch.mean(L_g * y_true_cls) + cls_loss
...
def crop_area(im, polys, crop_background=False, max_tries=5000, vis = False, img_name = None):
    '''
    make random crop from the input image
    :param im:
    :param polys:
